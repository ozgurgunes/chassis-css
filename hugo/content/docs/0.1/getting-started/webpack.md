---
layout: docs
title: Chassis - CSS and Webpack
description: The official guide for how to include and bundle Chassis - CSS's CSS and JavaScript in your project using Webpack.
group: getting-started
toc: true
thumbnail: guides/chassis-webpack@2x.png
---

<!--
<img class="d-block mx-auto mb-large image-fluid rounded-3" srcset="/docs/{{< param docs_version >}}/assets/img/guides/chassis-webpack.png, /docs/{{< param docs_version >}}/assets/img/guides/chassis-webpack@2x.png 2x" src="/docs/{{< param docs_version >}}/assets/img/guides/chassis-webpack.png" width="800" height="400" alt="">
-->
...
We're building a Webpack project with Chassis - CSS from scratch, so there are some prerequisites and upfront steps before we can really get started. This guide requires you to have Node.js installed and some familiarity with the terminal.
...
4. **Install additional dependencies.** In addition to Webpack and Chassis - CSS, we need a few more dependencies to properly import and bundle Chassis - CSS's CSS and JS with Webpack. These include Sass, some loaders, and Autoprefixer.
...
2. **Next we fill in our `src/index.html`.** This is the HTML page Webpack will load in the browser to utilize the bundled CSS and JS we'll add to it in later steps. Before we can do that, we have to give it something to render and include the `output` JS from the previous step.
   ...
   We're including a little bit of Chassis - CSS styling here with the `div class="container"` and `<button>` so that we see when Chassis - CSS's CSS is loaded by Webpack.
...
In the next and final section to this guide, we'll set up the Webpack loaders and import all of Chassis - CSS's CSS and JavaScript.

## Import Chassis - CSS

Importing Chassis - CSS into Webpack requires the loaders we installed in the first section. We've installed them with npm, but now Webpack needs to be configured to use them.
...
2. **Now, let's import Chassis - CSS's CSS.** Add the following to `src/scss/styles.scss` to import all of Chassis - CSS's source Sass.
...
3. **Next we load the CSS and import Chassis - CSS's JavaScript.** Add the following to `src/js/main.js` to load the CSS and import all of Chassis - CSS's JS. Popper will be imported automatically through Chassis - CSS.
...
4. **And you're done! 🎉** With Chassis - CSS's source Sass and JS fully loaded, your local development server should now look like this:
   ...
   ![Webpack dev server running with Chassis - CSS](/assets/img/guides/webpack-dev-server-chassis.png)

   Now you can start adding any Chassis - CSS components you want to use. Be sure to [check out the complete Webpack example project](https://github.com/twbs/examples/tree/main/webpack) for how to include additional custom Sass and optimize your build by importing only the parts of Chassis - CSS's CSS and JS that you need.
...
To separate the CSS so that we can load it directly from `dist/index.html`, use the `mini-css-extrlink-loader` Webpack plugin.

First, install the plugin:

```sh
npm install --save-dev mini-css-extrlink-plugin
```
...
@@ -3,6 +3,7 @@
 ...
 const autoprefixer = require('autoprefixer')
 const HtmlWebpackPlugin = require('html-webpack-plugin')
+const miniCssExtractPlugin = require('mini-css-extrlink-plugin')
