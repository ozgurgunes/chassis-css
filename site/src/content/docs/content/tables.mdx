...
<Code code={[
  ...
  `
<!-- On rows -->`,
  ...getData('context-colors').map((contextColor) => `<tr class="context ${contextColor.name}">...</tr>`),
  `
<!-- On cells (\`td\` or \`th\`) -->
<tr>`,
  ...getData('context-colors').map((contextColor) => `  <td class="context ${contextColor.name}">...</td>`),
  `</tr>`
]} lang="html" />
...
<Table class="table context alternate striped" />

<Table class="table context alternate striped-columns" />

<Table class="table context success striped" />

<Table class="table context success striped-columns" />
...
<div class="cxd-example">
  <table class="table context alternate">
    <thead>
      <tr>
        <th scope="col">#</th>
        <th scope="col">First</th>
...
- We start by setting the background of a table cell with the `--cx-table-bg` custom property. All table variants then set that custom property to colorize the table cells. This way, we don’t get into trouble if semi-transparent colors are used as table backgrounds.
- Then we add an inset box shadow on the table cells with `box-shadow: inset 0 0 0 9999px var(--cx-table-bg-state, var(--cx-table-bg-type, var(--cx-table-accent-bg)));` to layer on top of any specified `background-color`. It uses custom cascade to override the `box-shadow`, regardless the CSS specificity. Because we use a huge spread and no blur, the color will be monotone. Since `--cx-table-accent-bg` is set to `transparent` by default, we don’t have a default box shadow.
- When either `.table-striped`, `.table-striped-columns`, `.table-hover` or `.table-active` classes are added, either `--cx-table-bg-type` or `--cx-table-bg-state` (by default set to `initial`) are set to a semitransparent color (`--cx-table-striped-bg`, `--cx-table-active-bg` or `--cx-table-hover-bg`) to colorize the background and override default `--cx-table-accent-bg`.
- For each table variant, we generate a `--cx-table-accent-bg` color with the highest contrast depending on that color. For example, the accent color for `.table-primary` is darker while `.table-dark` has a lighter accent color.
...
<Table class="table context alternate small" />
