...
Width and height utilities are generated from the utility API in `scss/utilities/_sizing.scss`. Includes support for `25%`, `50%`, `75%`, `100%`, and `auto` by default. Modify those values as you need to generate different utilities here.
...
Sizing utilities are declared in our utilities API in `scss/utilities/_sizing.scss`. [Learn how to use the utilities API.]([[docsref:/utilities/api#using-the-api]])
