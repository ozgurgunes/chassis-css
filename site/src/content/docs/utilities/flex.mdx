...
Use `.flex-shrink-*` utilities to toggle a flex item's ability to shrink if necessary. In the example below, the second flex item with `.flex-shrink-0` cannot be forced to shrink even when space is limited, maintaining its original size even as the previous flex item with `.w-100` tries to take up more space.
...
Use the `.flex-fill` class on a series of sibling elements to force them into equal widths regardless of their content while taking up all available horizontal space.
...
Looking to replicate the [media object component](https://chassis-ui.com/docs/4.6/components/media-object/) from Chassis - CSS 4? Recreate it in no time with a few flex utilities that allow even more flexibility and customization than before.
...
Flexbox utilities are declared in our utilities API in `scss/utilities/_flex.scss`. [Learn how to use the utilities API.]([[docsref:/utilities/api#using-the-api]])
