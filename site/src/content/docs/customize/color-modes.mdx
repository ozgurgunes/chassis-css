---
title: Color modes
description: Chassis - CSS now supports color modes, or themes, as of v5.3.0. Explore our default light color mode and the new dark mode, or create your own using our styles as your template.
toc: true
---
...
Our new dark mode option is available to use for all users of Chassis, but it’s controlled via data attributes instead of media queries and does not automatically toggle your project’s color mode. You can disable our dark mode entirely via Sass by changing `$enable-dark-mode` to `false`.
...
<Example showMarkup={false} code={`<div class="cxd-example text-body bg-main" data-cx-theme="primary">
...
<Code containerClass="cxd-example-snippet" lang="js" filePath={getDocsRelativePath('/static/docs/[version]/assets/js/color-modes.js')} />

## Adding theme colors

Adding a new color in `$context-colors` is not enough for some of our components like [alerts]([[docsref:/components/alerts]]) and [list groups]([[docsref:/components/list-group]]). New colors must also be defined in `$context-colors-text`, `$context-colors-bg-subtle`, and `$context-colors-border-subtle` for light theme; but also in `$context-colors-text-dark`, `$context-colors-bg-subtle-dark`, and `$context-colors-border-subtle-dark` for dark theme.

This is a manual process because Sass cannot generate its own Sass variables from an existing variable or map. In future versions of Chassis, we'll revisit this setup to reduce the duplication.
...
// Light mode
$custom-colors-fg-main: ("custom-color": #712cf9);
$custom-colors-bg-evident: ("custom-color": #e1d2fe);
$custom-colors-border-subtle: ("custom-color": #bfa1fc);

$context-colors-fg-main: map-merge($context-colors-text, $custom-colors-text);
$context-colors-bg-evident: map-merge($context-colors-bg-subtle, $custom-colors-bg-subtle);
$context-colors-border-subtle: map-merge($context-colors-border-subtle, $custom-colors-border-subtle);

// Dark mode
$custom-colors-fg-main-dark: ("custom-color": #e1d2f2);
$custom-colors-bg-evident-dark: ("custom-color": #8951fa);
$custom-colors-border-subtle-dark: ("custom-color": #e1d2f2);

$context-colors-fg-main-dark: map-merge($context-colors-text-dark, $custom-colors-text-dark);
$context-colors-bg-evident-dark: map-merge($context-colors-bg-subtle-dark, $custom-colors-bg-subtle-dark);
$context-colors-border-subtle-dark: map-merge($context-colors-border-subtle-dark, $custom-colors-border-subtle-dark);
