...
Chassis - CSS has the option to enable a separate grid system that’s built on CSS Grid. You still get classes you can apply on a whim to build responsive layouts, but with a different approach under the hood.
...
These CSS variables have no default value; instead, they apply fallback values that are used _until_ a local instance is provided. For example, we use `var(--cx-rows, 1)` for our CSS Grid rows, which ignores `--cx-rows` because that hasn’t been set anywhere yet. Once it is, the `.grid` instance will use that value instead of the fallback value of `1`.
...
<Example class="cxd-example-cssgrid" code={`<div class="grid text-center" style="--cx-grid-columns: 18; --cx-grid-gap: .5rem;">
