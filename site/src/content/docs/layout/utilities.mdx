...
Chassis - CSS is built with flexbox, but not every element’s `display` has been changed to `display: flex` as this would add many unnecessary overrides and unexpectedly change key browser behaviors. Most of [our components]([[docsref:/components/notifications]]) are built with flexbox enabled.
...
Use the `margin` and `padding` [spacing utilities]([[docsref:/utilities/spacing]]) to control how elements and components are spaced and sized. Chassis - CSS includes a six-level scale for spacing utilities, based on a `1rem` value default `$spacer` variable. Choose values for all viewports (e.g., `.me-medium` for `margin-right: 1rem` in LTR), or pick responsive variants to target specific viewports (e.g., `.me-medium-3` for `margin-right: 1rem` —in LTR— starting at the `md` breakpoint).
