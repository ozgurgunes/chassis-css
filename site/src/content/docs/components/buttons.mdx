...
If you don’t want the button text to wrap, you can add the `.text-nowrap` class to the button. In Sass, you can set `$button-white-space: nowrap` to disable text wrapping for each button.
...
Use the `.icon-start` or `.icon-end` classes for proper spacing to add icons to a button. You can use icon font or SVG icon.

<Example code={`<button type="button" class="button default">
