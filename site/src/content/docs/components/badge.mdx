...
Enhance badges with icons to provide additional visual meaning. Icons help communicate purpose at a glance.

### Icon placement

Use helper classes for optimal icon placement and spacing. The `.icon-start` class positions the icon before text, while the `.icon-end` class positions the icon after text. Both SVG icons and icon fonts are supported:
