...
2. Either [download Chassis - CSS’s sources]([[config:download.source]]) or fork and clone [Chassis - CSS’s repository]([[config:repo]]).
3. Navigate to the root `/chassis` directory and run `npm install` to install our local dependencies listed in [package.json]([[config:repo]]/blob/v[[config:current_version]]/package.json).
