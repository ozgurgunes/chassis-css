---
title: Docs reference
description: Examples of Chassis - CSS’s documentation-specific components and styles.
aliases: "/docsref/"
toc: true
robots: noindex,follow
---

## Buttons

<button class="button cxd-primary">Primary button</button>
<button class="button cxd-accent">Accent button</button>
<button class="button cxd-light">Light button</button>

## Callouts

<Callout>
  Default callout
</Callout>

<Callout type="warning">
  Warning callout
</Callout>

<Callout type="danger">
  Danger callout
</Callout>

## Code example

```scss
.test {
  --color: blue;
}
```

<div class="cxd-example">
  The <abbr title="HyperText Markup Language">HTML</abbr> abbreviation element.
</div>

<Example code={`<div class="test">This is a test.</div>`} />

<ScssDocs name="variable-gradient" file="scss/_variables.scss" />

<JsDocs name="live-toast" file="site/src/assets/partials/snippets.js" />
