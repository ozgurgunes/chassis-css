...
Since Chassis - CSS applies `display: block` and `width: 100%` to almost all our form inputs, forms will by default stack vertically. Additional classes can be used to vary this layout on a per-form basis.

## Utilities

[Margin utilities]([[docsref:/utilities/spacing]]) are the easiest way to add some structure to forms. They provide basic grouping of labels, controls, optional form assist, and form validation messaging. We recommend sticking to `margin-bottom` utilities, and using a single direction throughout the form for consistency.
...
Be sure to use `.col-form-label-small` or `.col-form-label-large` to your `<label>`s or `<legend>`s to correctly follow the size of `.form-input-large` and `.form-input-small`.
...
Use the `.row-cols-*` classes to create responsive horizontal layouts. By adding [gutter modifier classes]([[docsref:/layout/gutters]]), we'll have gutters in horizontal and vertical directions. On narrow mobile viewports, the `.col-12` helps stack the form inputs and more. The `.align-items-center` aligns the form elements to the middle, making the `.form-option` align properly.
