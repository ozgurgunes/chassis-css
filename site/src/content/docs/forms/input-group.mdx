...
Chassis CSS's design token system allows input groups to adapt to different themes and styling needs while maintaining consistent behavior.

### Alternate styling

Apply the `.form-alternate` class to input groups to use Chassis CSS's alternate form styling, perfect for different visual contexts or emphasis needs.

<Example code={`<div class="row g-xlarge">
    <div class="col">
      <label for="default-input-group" class="form-label">Default style</label>
      <div class="input-group mb-medium">
        <span class="input-addon">@</span>
    ...
    <div class="col">
      <label for="alternate-input-group" class="form-label">Alternate style</label>
...
The alternate styling applies different background colors, border styles, and text colors throughout the input group while maintaining proper visual relationships between elements. This works with all addon types and form controls.

### Theme and color mode support

Input groups fully support Chassis CSS's theming system, including dark mode. Colors, borders, and contrast are automatically adjusted based on the current theme and color mode.

Form validation states are also properly handled with input groups, with validation feedback placed outside the group for optimal visual hierarchy and accessibility.
...
Input groups use these CSS variables for styling, making them highly customizable:

<ScssDocs name="input-group-variables" file="scss/_variables.scss" />

You can override these variables in your custom CSS to adapt input groups to your design system:

```css
.custom-input-group {
  --cx-input-addon-fg-color: #2563eb;
  --cx-input-addon-bg-color: #eff6ff;
  --cx-input-addon-border-color: #93c5fd;
}
```
