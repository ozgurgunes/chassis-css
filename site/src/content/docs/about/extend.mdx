...
While the getting started pages provide an introductory tour of the project and what it offers, this document focuses on _why_ we do the things we do in Chassis. It explains our philosophy to building on the web so that others can learn from us, contribute with us, and help us improve.
...
As such, components should be built with a base class that houses common, not-to-be overridden property-value pairs. For example, `.button` and `.button-primary`. We use `.button` for all the common styles like `display`, `padding`, and `border-width`. We then use modifiers like `.button-primary` to add the color, background-color, border-color, etc.
...
There are two `z-index` scales in Chassis—elements within a component and overlay components.
...
[Code Guide](https://codeguide.co/) (from Chassis - CSS co-creator, @mdo) documents how we write our HTML and CSS across Chassis. It specifies guidelines for general formatting, common sense defaults, property and attribute orders, and more.

We use [Stylelint](https://stylelint.io/) to enforce these standards and more in our Sass/CSS. [Our custom Stylelint config](https://github.com/twbs/stylelint-config-twbs-chassis) is open source and available for others to use and extend.
