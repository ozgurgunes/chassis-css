...
- Dropped recently added form input background CSS variables and reassigned the Sass variables to use CSS variables instead. This simplifies the styling across color modes and avoids an issue where form inputs in dark mode wouldn’t update properly.
...
- <span class="badge text-warning-emphasis bg-warning-subtle">Deprecated</span> The `.text-muted` utility and `$text-muted` Sass variable have been deprecated and replaced with `.fg-subtle` and `$body-fg-subtle`.
...
- **Global support for light (default) and dark color modes.** Set color mode globally on the `:root` element, on groups of elements and components with a wrapper class, or directly on components, with `data-cx-theme="light|dark"`. Also included is a new `color-mode()` mixin that can output a ruleset with the `data-cx-theme` selector or a media query, depending on your preference.
  ...
  <span class="badge text-warning-emphasis bg-warning-subtle">Deprecated</span> Color modes replace dark variants for components, so  `.close-button-white`, `.carousel-dark`, `.dropdown-menu-dark`, and `.navbar-dark` are deprecated.

- **New extended color system.** We’ve added new theme colors (but not in `$context-colors`) for a more nuanced, system-wide color palette with new secondary, tertiary, and emphasis colors for `color` and `background-color`. These new colors are available as Sass variables, CSS variables, and utilities.
...
- Adds additional variables for alerts, `.close-button`, and `.offcanvas`.
...
- <span class="badge text-warning-emphasis bg-warning-subtle">Deprecated</span> The `list-item-variant()` mixin is now deprecated. We now [use a Sass loop]([[docsref:/components/list-group#sass-loops]]) directly to modify the component’s default CSS variables for each variant.
...
- <span class="badge text-warning-emphasis bg-warning-subtle">Deprecated</span> The `.close-button-white` class has been deprecated and replaced with `data-cx-theme="dark"` on the close button or any parent element. [See the docs for an example.]([[docsref:/components/close-button#dark-variant]])
...
- `.form-input` is now styled with CSS variables to support color modes. This includes the addition of two new root CSS variables for the default and disabled form input backgrounds.
...
- <span class="badge text-warning-emphasis bg-warning-subtle">Deprecated</span> `.text-muted` will be replaced by `.fg-subtle` in v6.

  With the addition of the expanded theme colors and variables, the `.text-muted` variables and utility have been deprecated with v5.3.0. Its default value has also been reassigned to the new `--cx-fg-subtle` CSS variable to better support color modes. It will be removed in v6.0.0.
...
For a complete list of changes, [see the v5.3.0 project on GitHub](https://github.com/twbs/chassis-css/projects/).
...
+ $context-colors: map-merge($context-colors, $custom-context-color);
+
...
- **[Scrollspy has been rewritten](https://github.com/twbs/chassis-css/pull/33421) to use the Intersection Observer API**, which means you no longer need relative parent wrappers, deprecates `offset` config, and more. Look for your Scrollspy implementations to be more accurate and consistent in their nav highlighting.
...
For a complete list of changes, [see the v5.2.0 project on GitHub](https://github.com/twbs/chassis-css/projects/32).
...
Want more information? [Read the v5.1.0 blog post.](https://blog.chassis-ui.com/2021/08/04/chassis-5-1-0/)
...
- Added [new Customize section]([[docsref:/customize/overview/]]), replacing [v4’s Theming page](https://chassis-ui.com/docs/4.6/getting-started/theming/), with new details on Sass, global configuration options, color schemes, CSS variables, and more.
...
- We’ve ditched the default Sass map merges to make it easier to remove redundant values. Keep in mind you now have to define all values in the Sass maps like `$context-colors`. Check out how to deal with [Sass maps]([[docsref:/customize/sass#maps-and-loops]]).

- <span class="badge text-bg-danger">Breaking</span> Renamed `color-yiq()` function and related variables to `color-contrast()` as it’s no longer related to YIQ color space. [See #30168.](https://github.com/twbs/chassis-css/pull/30168/)
...
- <span class="badge text-bg-danger">Breaking</span> Removed print styles and `$enable-print-styles` variable. Print display classes are still around. [See #28339](https://github.com/twbs/chassis-css/pull/28339).

- <span class="badge text-bg-danger">Breaking</span> Dropped `color()`, `theme-color()`, and `gray()` functions in favor of variables. [See #29083](https://github.com/twbs/chassis-css/pull/29083).

- <span class="badge text-bg-danger">Breaking</span> Renamed `theme-color-level()` function to `color-level()` and now accepts any color you want instead of only `$theme-color` colors. [See #29083](https://github.com/twbs/chassis-css/pull/29083) **Watch out:** `color-level()` was later on dropped in `v5.0.0-alpha3`.
...
- <span class="badge text-bg-danger">Breaking</span> Dropped the `.media` component as it can be easily replicated with utilities. [See #28265](https://github.com/twbs/chassis-css/pull/28265) and the [flex utilities page for an example]([[docsref:/utilities/flex#media-object]]).
...
- `$enable-grid-classes` no longer disables the generation of container classes anymore. [See #29146.](https://github.com/twbs/chassis-css/pull/29146)
...
- <span class="badge text-bg-danger">Breaking</span> Dropped `.pre-scrollable` class. [See #29135](https://github.com/twbs/chassis-css/pull/29135)

- <span class="badge text-bg-danger">Breaking</span> `.text-*` utilities do not add hover and focus states to links anymore. `.link-*` helper classes can be used instead. [See #29267](https://github.com/twbs/chassis-css/pull/29267)

- <span class="badge text-bg-danger">Breaking</span> Dropped `.text-justify` class. [See #29793](https://github.com/twbs/chassis-css/pull/29793)

- <span class="badge text-bg-danger">Breaking</span> ~~`<hr>` elements now use `height` instead of `border` to better support the `size` attribute. This also enables use of padding utilities to create thicker dividers (e.g., `<hr class="py-2xsmall">`).~~

- Reset default horizontal `padding-left` on `<ul>` and `<ol>` elements from browser default `40px` to `2rem`.

- Added `$enable-smooth-scroll`, which applies `scroll-behavior: smooth` globally—except for users asking for reduced motion through `prefers-reduced-motion` media query. [See #31877](https://github.com/twbs/chassis-css/pull/31877)
...
- <span class="badge text-bg-danger">Breaking</span> **Consolidated native and custom form elements.** Checkboxes, radios, selects, and other inputs that had native and custom classes in v4 have been consolidated. Now nearly all our form elements are entirely custom, most without the need for custom HTML.
  ...
  - `.custom-file` and `.form-input-file` have been replaced by custom styles on top of `.form-input`.
  - `.custom-range` is now `.form-range`.
  - Dropped native `.form-input-file` and `.form-input-range`.
...
- The longstanding [Missing border radius on input group with validation feedback bug](https://github.com/twbs/chassis-css/issues/25110) is finally fixed by adding an additional `.has-validation` class to input groups with validation.
...
- <span class="badge text-bg-danger">Breaking</span> `.form-assist` no longer sets `display`, allowing you to create inline or block help text as you wish just by changing the HTML element.

- Form inputs no longer used fixed `height` when possible, instead deferring to `min-height` to improve customization and compatibility with other components.
...
- Unified `padding` values for alerts, breadcrumbs, cards, dropdowns, list groups, modals, popovers, and tooltips to be based on our `$spacer` variable. [See #30564](https://github.com/twbs/chassis-css/pull/30564).
...
- <span class="badge text-bg-danger">Breaking</span> **[Toggle buttons]([[docsref:/forms/checks-radios#toggle-buttons]]), with checkboxes or radios, no longer require JavaScript and have new markup.** We no longer require a wrapping element, add `.button-option` to the `<input>`, and pair it with any `.button` classes on the `<label>`. [See #30650](https://github.com/twbs/chassis-css/pull/30650). _The docs for this has moved from our Buttons page to the new Forms section._
...
- <span class="badge text-bg-danger">Breaking</span> Dropped `.card-columns` in favor of Masonry. [See #28922](https://github.com/twbs/chassis-css/pull/28922).
...
- Replaced chevron icons for carousel controls with new SVGs from [Chassis - CSS Icons]([[config:icons]]).

#### Close button

- <span class="badge text-bg-danger">Breaking</span> Renamed `.close` to `.close-button` for a less generic name.

- Close buttons now use a `background-image` (embedded SVG) instead of a `&times;` in the HTML, allowing for easier customization without the need to touch your markup.

- Added new `.close-button-white` variant that uses `filter: invert(1)` to enable higher contrast dismiss icons against darker backgrounds.
...
- Spinners now honor `prefers-reduced-motion: reduce` by slowing down animations. [See #31882](https://github.com/twbs/chassis-css/pull/31882).
...
- Added new `.bg-main` class for quickly setting the `<body>`’s background to additional elements.
...
- <span class="badge text-bg-danger">Breaking</span> Removed `.rounded-small` and `rounded-large`, and introduced a new scale of classes, `.rounded-0` to `.rounded-3`. [See #31687](https://github.com/twbs/chassis-css/pull/31687).
...
- <span class="badge text-bg-danger">Breaking</span> **"Screen reader" classes are now ["visually hidden" classes]([[docsref:/utilities/visibility#visually-hidden]]).**
