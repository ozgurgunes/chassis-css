...
layout: docs
title: Popovers
description: Documentation and examples for adding Chassis - CSS popovers, like those found in iOS, to any element on your site.
group: components
toc: true
---
...
- Popovers rely on the third party library [Popper](https://popper.js.org/docs/v2/) for positioning. You must include [popper.min.js]({{< param "cdn.popper" >}}) before `chassis.js`, or use one `chassis.bundle.min.js` which contains Popper.
...
As mentioned above, you must initialize popovers before they can be used. One way to initialize all popovers on a page would be to select them by their `data-cx-toggle` attribute, like so:
...
We use JavaScript similar to the snippet above to render the following live popover. Titles are set via `data-cx-title` and body content is set via `data-cx-content`.
...
<button type="button" class="button large danger" data-cx-toggle="popover" data-cx-title="Popover title" data-cx-content="And here's some amazing content. It's very engaging. Right?">Click to toggle popover</button>
{{< /example >}}

### Four directions

Four options are available: top, right, bottom, and left. Directions are mirrored when using Chassis - CSS in RTL. Set `data-cx-placement` to change the direction.
...
You can customize the appearance of popovers using [CSS variables](#variables). We set a custom class with `data-cx-custom-class="custom-popover"` to scope our custom appearance and use it to override some of the local CSS variables.
...
{{< example stackblitz_add_js="true" >}}
<a tabindex="0" class="button large button-danger" role="button" data-cx-toggle="popover" data-cx-trigger="focus" data-cx-title="Dismissible popover" data-cx-content="And here's some amazing content. It's very engaging. Right?">Dismissible popover</a>
{{< /example >}}
...
For disabled popover triggers, you may also prefer `data-cx-trigger="hover focus"` so that the popover appears as immediate visual feedback to your users as they may not expect to _click_ on a disabled element.
