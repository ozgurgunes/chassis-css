...
(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.chassis = factory());
  ...
  const SELECTOR_DATA_TOGGLE$4 = '[data-cx-toggle="collapse"]';
  const Default$a = {
    parent: null,
    toggle: true
  };
  ...
  */
  ...
  const SELECTOR_DATA_TOGGLE$3 = '[data-cx-toggle="dropdown"]:not(.disabled):not(:disabled)';
  const SELECTOR_DATA_TOGGLE_SHOWN = `${SELECTOR_DATA_TOGGLE$3}.${CLASS_NAME_SHOW$7}`;
  ...
  class Dropdown extends BaseComponent {
    ...
    _getPlacement() {
      ...
      const isEnd = getComputedStyle(this._menu).getPropertyValue('--cx-position').trim() === 'end';
  ...
  const EVENT_MOUSEDOWN = `mousedown.cx.${NAME$a}`;
  const Default$8 = {
  ...
  const SELECTOR_CONTAINER = '.modal-container';
  const SELECTOR_MODAL_CONTENT = '.content';
  const SELECTOR_DATA_TOGGLE$2 = '[data-cx-toggle="modal"]';
  const Default$6 = {
    backdrop: true,
    focus: true,
    keyboard: true
  ...
  class Modal extends BaseComponent {
    constructor(element, config) {
      super(element, config);
      this._dialog = SelectorEngine.findOne(SELECTOR_CONTAINER, this._element);
    ...
    _showElement(relatedTarget) {
      ...
      const modalBody = SelectorEngine.findOne(SELECTOR_MODAL_CONTENT, this._dialog);
      if (modalBody) {
  ...
  const NAME$7 = 'notification';
  const DATA_KEY$4 = 'cx.notification';
  ...
  const SELECTOR_DATA_TOGGLE$1 = '[data-cx-toggle="offcanvas"]';
  const Default$5 = {
    backdrop: true,
    keyboard: true,
    scroll: false
...
//# sourceMappingURL=chassis.bundle.js.map
